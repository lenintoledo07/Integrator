...
async function getUnifiedTokens(chainId) {
    ...
    let tokenMap = {};
    try {
        const tokens = chainId === networks_1.ChainId.metis ? await getMetisTokens() : await get1InchTokens(chainId);
...
async function get1InchTokens(chainId) {
    ...
    const tokens = Object.values(data).map(({ address, decimals, symbol, name, logoURI }) => address === elasticAddress ? nativeToken : new token_1.Token(chainId, address, decimals, symbol, name, logoURI));
    return tokens;
}
